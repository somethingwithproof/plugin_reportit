...
if ($schedule == true) {
	$pending = db_fetch_assoc_prepared(
		'SELECT *
		FROM reports_queued
		WHERE status = ?
		AND source = ?',
...
function runtime($report_id, $queue_id, $start_time = 0)
{
	...
	for ($i = 0; $i < $number_of_rrds; $i++) {
		...
		$rrd_ad_data = get_type_of_request(
			$shift_startday,
			$shift_endday,
			$f_sp,
			$l_sp,
		...
		foreach ($rrd_data as $rra_index => $data) {
			$pre_data[$rra_index] = get_prepared_data(
...
function autorrdlist($reportid)
{
	...
	$current_rows = db_fetch_cell_prepared(
		'SELECT COUNT(*)
...
function autocleanup($report_id)
{
	...
	if ($data_items) {
		db_execute_prepared(
			"DELETE FROM `plugin_reportit_data_items`
...
function autoexport($report_id)
{
	...
	$template_folder = db_fetch_cell_prepared(
		'SELECT b.export_folder
		FROM plugin_reportit_reports AS a
		INNER JOIN plugin_reportit_templates as b
		ON a.template_id = b.id
		WHERE a.id = ?',
